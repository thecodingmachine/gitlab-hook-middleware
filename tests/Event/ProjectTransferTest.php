...
class ProjectTransferTest extends TestCase {
     ...
     * @return mixed[]
     */
    private function getData($filename = 'project_transfer'): array {
        $data = file_get_contents(__DIR__ . '/../fixtures/'.$filename.'.json');
        return json_decode($data, true);
    }

    public function testGetters() {
        $event = new ProjectTransfer($this->getData());
        ...
        $this->assertSame('scores/underscore', $event->getPathWithNamespace());
        $this->assertSame(73, $event->getProjectId());
        $this->assertSame('internal', $event->getProjectVisibility());
        $this->assertSame('jsmith/overscore', $event->getOldPathWithNamespace());
    }
